...
public class PerforceNumberNameSynchronizer implements PersistentStateComponent<PerforceNumberNameSynchronizer.ConfigBean> {
  ...
  public Collection<Long> findOrCreate(final @NotNull P4Connection connection, final LocalChangeList list) throws VcsException {
   ...
   */
  private static @NotNull String obtainIdeaChangeList(@NotNull ChangeListManagerGate gate,
  ...
  private static @NotNull String resetFromNativeDescription(ChangeListManagerGate gate,
  ...
  private static @NotNull String suggestUniqueChangeListName(ChangeListManagerGate gate, String baseName) {
    return UniqueNameGenerator.generateUniqueName(baseName, "", "", " (", ")", s -> gate.findChangeList(s) == null);
  }

  @VisibleForTesting
  public static @NotNull String toOneLine(String description) {
  ...
  public @Nullable Long getNumber(@NotNull ConnectionKey key, @NotNull String name) {
    synchronized (myLock) {
      PerforceNumberNameMap map = myMap.get(key);
  ...
  @Override
  public @Nullable ConfigBean getState() {
    synchronized (myLock) {
